...
int getToken(void);
int func_ptr = -1; /* 現在作業している関数のテーブル位置 */
...
void parse_Block() { /* PL/0' 最終版の文法に対応 */
  ...
  parse_ConstDecl();
  parse_VarDecl();
  backpatch_main = gencode_arg_V(jmp, 0); /* 関数定義を飛ばす */
  parse_FuncDecl();
  backpatch(backpatch_main); /* jpc を書き換え */
  parse_Statement();
}
...
void parse_ConstIdList() {
  ...
  if (func_ptr > -1) gencode_no_arg(pushup); /* 関数内なら領域確保 */
...
void parse_FuncDecl() {
  ...
  /* 関数名の記号表への登録 */
  func_ptr = register_func_in_tbl(yytext, line_no);
  set_func_address(func_ptr, next_code());
  nextToken = getToken();
  ...
  end_param(func_ptr); /* 仮引数の領域再計算 */
  nextToken = getToken();
  /* 関数前処理 */
  gencode_no_arg(enterf);
  parse_FuncBlock();
  if (nextToken != T_SEMIC) pl0_error("構文", yytext, line_no, ";がない。");
  /* 関数後処理 */
...
void parse_Statement() {
  ...
  if (nextToken == T_ID) { /* 代入文 */
    ...
    if (func_ptr < 0) pl0_error("意味", yytext, line_no, "return はダメです。");
    nextToken = getToken();
    parse_Expression();
    /* ここで return のコード生成 */
    gencode_no_arg(leavef);
  ...
  if (nextToken == T_ID) { 
    ...
    if (ptr == 0) { /* なかった場合 */
      ...
      if (nextToken != T_LPAR) pl0_error("構文", yytext, line_no, "(がない。");
        ...
        /* 関数の呼出コード生成 */
	gencode_arg_V(call, get_symbol_address(ptr));
        
      }
    } else {
      /* 関数以外(定数/変数/仮引数)の場合のコード生成 */ 
      /* info_ref_symbol(ptr, line_no); */
