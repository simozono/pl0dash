...
void info_ref_symbol(int ptr, int cur_line) {
  ...
  addr = get_symbol_address(ptr);
  
  switch (get_symbol_type(ptr)) {
  case const_id:
    fprintf(stderr, "%d行目で使われている\"%s\"は%d行目で定義された[定数]です。アドレスは%dです\n",
	    cur_line, id, def_line, addr);
    break;
  case var_id:
    fprintf(stderr, "%d行目で使われている\"%s\"は%d行目で定義された[変数]です。アドレスは%dです\n",
	    cur_line, id, def_line, addr);
    break;
  case param_id:
    fprintf(stderr, "%d行目で使われている\"%s\"は%d行目で定義された[仮引数]です。アドレスは%dです\n",
	    cur_line, id, def_line, addr);
    break;
  case func_id:
    fprintf(stderr, "%d行目で使われている\"%s\"は%d行目で定義された[関数]です。アドレスは%dです\n",
	    cur_line, id, def_line, addr);
    break;
  }
}
